...
Private m_ButtonImagesPressed As pdDIB
...
Public Sub AssignImage(Optional ByRef resName As String = vbNullString, Optional ByRef srcDIB As pdDIB = Nothing, Optional ByVal useImgWidth As Long = 0, Optional ByVal useImgHeight As Long = 0, Optional ByVal imgBorderSizeIfAny As Long = 0)
    ...
    If (m_ButtonWidth <> 0) And (m_ButtonHeight <> 0) Then
        ...
        GenerateVariantButtonImages m_ButtonImages
...
'Assign an *OPTIONAL* special DIB to this button, to be used only when the button is pressed.  Disabled and hover state are
' automatically generated for you.
'
'IMPORTANT NOTE!  To reduce resource usage, PD requires that this optional "pressed" image have identical dimensions to the
' primary image. This greatly simplifies layout and painting issues, so I do not expect to change it.
'
'Note that you can supply an existing DIB, or a resource name.  You must supply one or the other (obviously).
Public Sub AssignImage_Pressed(Optional ByVal resName As String = vbNullString, Optional ByRef srcDIB As pdDIB, Optional ByVal useImgWidth As Long = 0, Optional ByVal useImgHeight As Long = 0, Optional ByVal imgBorderSizeIfAny As Long = 0)
    ...
    If (m_ButtonWidth <> 0) And (m_ButtonHeight <> 0) Then
        ...
        'Create our vertical sprite-sheet DIB, and mark it as having premultiplied alpha
        If (m_ButtonImagesPressed Is Nothing) Then Set m_ButtonImagesPressed = New pdDIB
        m_ButtonImagesPressed.CreateBlank m_ButtonWidth, m_ButtonHeight * 3, srcDIB.GetDIBColorDepth, 0, 0
        m_ButtonImagesPressed.SetInitialAlphaPremultiplicationState False
        
        'Copy the normal DIB into place at the top of the sheet
        GDI.BitBltWrapper m_ButtonImagesPressed.GetDIBDC, 0, 0, m_ButtonWidth, m_ButtonHeight, srcDIB.GetDIBDC, 0, 0, vbSrcCopy
        
        'A separate function will automatically generate "glowy hovered" and "grayscale disabled" versions for us
        GenerateVariantButtonImages m_ButtonImagesPressed
        
        'Reset alpha premultiplication and release this DIB from its DC (as it may not be rendered right away)
        m_ButtonImagesPressed.SetAlphaPremultiplication True
        If (LenB(resName) = 0) And initAlphaState Then srcDIB.SetAlphaPremultiplication True
        m_ButtonImagesPressed.FreeFromDC
...
Private Sub GenerateVariantButtonImages(ByRef srcDIB As pdDIB)
...
Private Sub RedrawBackBuffer(Optional ByVal raiseImmediateDrawEvent As Boolean = False, Optional ByVal testMouseState As Boolean = True)
    ...
    If PDMain.IsProgramRunning() Then
        ...
        If (m_ButtonWidth <> 0) Then
            ...
            If Me.Enabled Then
                If Value And (Not m_ButtonImagesPressed Is Nothing) Then
                    If ucSupport.IsMouseInside Or ucSupport.DoIHaveFocus Then
                        m_ButtonImagesPressed.AlphaBlendToDCEx bufferDC, btImageCoords.x, btImageCoords.y, m_ButtonWidth, m_ButtonHeight, 0, m_ButtonHeight, m_ButtonWidth, m_ButtonHeight
                    Else
                        m_ButtonImagesPressed.AlphaBlendToDCEx bufferDC, btImageCoords.x, btImageCoords.y, m_ButtonWidth, m_ButtonHeight, 0, 0, m_ButtonWidth, m_ButtonHeight
                    End If
                Else
                    If ucSupport.IsMouseInside Or ucSupport.DoIHaveFocus Then
            ...
            Else
                If Value And (Not m_ButtonImagesPressed Is Nothing) Then
                    m_ButtonImagesPressed.AlphaBlendToDCEx bufferDC, btImageCoords.x, btImageCoords.y, m_ButtonWidth, m_ButtonHeight, 0, m_ButtonHeight * 2, m_ButtonWidth, m_ButtonHeight
                Else
            ...
            m_ButtonImages.FreeFromDC
            If (Not m_ButtonImagesPressed Is Nothing) Then m_ButtonImagesPressed.FreeFromDC
