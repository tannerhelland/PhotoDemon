VERSION 5.00
Begin VB.UserControl pdComboBox_Hatch 
   Appearance      =   0  'Flat
   BackColor       =   &H00FFFF80&
   ClientHeight    =   975
   ClientLeft      =   0
   ...
   End
End
Attribute VB_Name = "pdComboBox_Hatch"
...
'
'A few notes on this hatch selection drop-down (combo) box control, specifically:
'
...
'    nearly identical.
' 2) This UC does provide any control over its list contents (e.g. there is no "AddItem" function).
...
'Hatch size is constant, regardless of OS
...
Public Sub initializeHatchList()

    'Clear the existing list, if any
    Me.Clear
    
    'For accessibility reasons, copy a numeric version of the hatch to the system combo box
...
'Populate the combo box with numeric representations of each hatch.  We don't actually use this data, but this provides a fallback
' for accessibility technology.
Private Sub copyHatchesToComboBox()
    ...
    If (m_ComboBoxHwnd <> 0) Then
        ...
        'Add a fake string to each entry
        Dim i As Long
        For i = 0 To NUM_OF_HATCHES - 1
            ...
            SendMessage m_ComboBoxHwnd, CB_INSERTSTRING, i, ByVal StrPtr(tmpString)
...
Private Sub dynamicallyFitDropDown(ByVal listHwnd As Long)
    ...
    If m_ComboBoxHwnd <> 0 Then
        ...
        If NUM_OF_HATCHES > 8 Then
            totalHeight = (m_ItemHeight * 2 + 2) * 8
        Else
            totalHeight = (m_ItemHeight * 2 + 2) * NUM_OF_HATCHES
        End If
...
Public Function ListCount() As Long
    ...
    'ListCount is hard-coded for this hatch-specific dropdown
    ListCount = NUM_OF_HATCHES
    
End Function

'Retrieve a specified list item
Public Property Get List(ByVal indexOfItem As Long) As String
    List = CStr(indexOfItem)
End Property
    ...
    If m_ComboBoxHwnd <> 0 Then
        ...
        If GetComboBoxInfo(m_ComboBoxHwnd, cbiCombo) <> 0 Then
            ...
            If targetDC <> 0 Then
                ...
                Dim tmpString As String
                tmpString = CStr(m_CurrentListIndex + 1) & " - "
                ...
                If Not curFont Is Nothing Then
                    ...
                    Dim tmpRect As RECT
                    With tmpRect
                        .Left = cbiCombo.rcItem.Left + 4
                        .Top = cbiCombo.rcItem.Top
                        .Right = (cbiCombo.rcItem.Right - cbiCombo.rcItem.Left)
                        .Bottom = (cbiCombo.rcItem.Bottom - cbiCombo.rcItem.Top) - 2
                    End With
                ...
                With hatchRect
                    .Left = cbiCombo.rcItem.Left + 4 + m_LargestWidth
                    .Top = cbiCombo.rcItem.Top
                    .Height = (cbiCombo.rcItem.Bottom - cbiCombo.rcItem.Top) - 1
                    .Width = (cbiCombo.rcItem.Right - .Left) - 4
                End With
...
Private Function DrawComboBoxEntry(ByRef srcDIS As DRAWITEMSTRUCT) As Boolean
    ...
    If srcDIS.CtlType = ODT_COMBOBOX Then
        ...
        If (srcDIS.itemID <> -1) Then
            ...
            'Retrieve the string for the active combo box entry.
            Dim tmpString As String
            tmpString = CStr(srcDIS.itemID + 1) & " - "
            ...
            With hatchRect
                .Left = srcDIS.rcItem.Left + 4 + m_LargestWidth
                .Top = srcDIS.rcItem.Top + 2
                .Height = (srcDIS.rcItem.Bottom - srcDIS.rcItem.Top) - 5
                .Width = (srcDIS.rcItem.Right - .Left) - 4
            End With
...
Public Sub SetWidthAutomatically(Optional ByVal newWidth As Long = 100)
