...
Private prevLayerID As Long, prevSettingID As Long, prevSettingValue As Variant
...
' This function will make a note of that value, which can easily be compared when the control loses focus.
Public Sub flagInitialNDFXState_Text(ByVal textSettingID As PDLAYER_TEXT_PROPERTY, ByVal textSettingValue As Variant, ByVal targetLayerID As Long)
    ...
    prevLayerID = targetLayerID
    prevSettingID = textSettingID
    prevSettingValue = textSettingValue
    
End Sub

'When a control tied to a non-destructive effect loses focus, it should call this function with its current value (translated as appropriate).
' This function will compare the value against its previously stored value, and if the two do not match, this function will add an Undo entry
...
Public Sub flagFinalNDFXState_Text(ByVal textSettingID As PDLAYER_TEXT_PROPERTY, ByVal textSettingValue As Variant)
    ...
    If textSettingID = prevSettingID Then
        ...
        'Next, see if the new setting value differs.  If it does, we need to update the Undo/Redo chain and the Macro recorder list
        ' (if they're currently being recorded, obviously)
        If StrComp(CStr(textSettingValue), CStr(prevSettingValue), vbBinaryCompare) <> 0 Then
            
            Debug.Print "OLD VALUE: " & CStr(prevSettingValue)
            Debug.Print "NEW VALUE: " & CStr(textSettingValue)
            
            'Raise a generic "text setting change" processor request
            MiniProcess_NDFXOnly "Modify text layer", , buildParams(textSettingID, textSettingValue), UNDO_LAYER_VECTORSAFE, , , prevLayerID
        
        End If
    
    Else
        Debug.Print "WARNING!  flagFinalNDFXState_Text was passed an mismatched textSettingID value.  Fix this!"
    End If
...
Private Sub MiniProcess_NDFXOnly(ByVal processID As String, Optional showDialog As Boolean = False, Optional processParameters As String = "", Optional createUndo As PD_UNDO_TYPE = UNDO_NOTHING, Optional relevantTool As Long = -1, Optional recordAction As Boolean = True, Optional ByVal targetLayerID As Long = -1)
    ...
    Select Case processID
        ...
        Case "Modify text layer"
            ...
            'The command has already been processed, so we don't need to do any further image processing.
            ' Instead, we append the name of this text layer operation so we can supply better Undo/Redo text to the user.
...
Private Function getNameOfTextAction(ByVal textSettingID As PDLAYER_TEXT_PROPERTY) As String
    ...
    Select Case textSettingID
        ...
        Case ptp_TextAntialiasing
            getNameOfTextAction = g_Language.TranslateMessage("text: antialiasing")
        
        Case ptp_TextContrast
            getNameOfTextAction = g_Language.TranslateMessage("text: clarity")
