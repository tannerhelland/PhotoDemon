...
Private Sub RemoveSelectionAsNecessary(ByVal processID As String, Optional raiseDialog As Boolean = False, Optional processParameters As String = vbNullString, Optional createUndo As PD_UndoType = UNDO_Nothing)
    ...
    If (Not raiseDialog) And PDImages.IsImageActive() Then
        ...
        If PDImages.GetActiveImage.IsSelectionActive And (createUndo <> UNDO_Selection) Then
            ...
            'We also need to catch another strange occurrence here.  PD's "Crop" command forcibly
            ' clears the selection upon completion.  This is done as a convenience because after
            ' cropping, an active selection is unlikely to be useful.  Unfortunately, this behavior
            ' wreaks havoc on PD's Undo/Redo engine, because the Undo/Redo engine only saves image
            ' state *after* an action has completed.  So the image's state post-Crop is saved nicely,
            ' but pre-Crop it may not be, because the selection is removed out-of-process.
